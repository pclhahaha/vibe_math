...
const f=(x)=>{if(fnType===0)return Math.sqrt(x)+0.3*Math.sin(3*x);if(fnType===1)return x;if(fnType===2)return 1/(1+Math.exp(-4*(x-2)));return 0;};
const fl=()=>{if(fnType===0)return '√x+0.3sin(3x)';if(fnType===1)return 'x (linear)';if(fnType===2)return 'σ(x-2) sigmoid';return '';};
p.setup=()=>{
...
p.stroke('#58a6ff');p.strokeWeight(2.5);p.noFill();p.beginShape();for(let px=0;px<=w;px+=2){const x=p.map(px,0,w,0,4);p.vertex(px,h-f(x)*h/4);}p.endShape();
...
p.fill(248,113,102,25);p.noStroke();p.rect(0,h-(fx+eps)*h/4,w,2*eps*h/4);
p.fill('#f78166');p.noStroke();p.circle(px0,h-fx*h/4,8);
p.fill('#8b949e');p.textSize(14);p.text('eps='+eps.toFixed(2)+' delta='+delta.toFixed(2)+' | fn='+fl(),20,25);
p.text('|x-x0|<delta => |f(x)-f(x0)|<eps — Real Analysis (Cauchy, Weierstrass)',20,48);
